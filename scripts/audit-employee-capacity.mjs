...
const query = readFileSync(resolve("supabase", "snippets", "employee_capacity_preflight.sql"), "utf8");
...
if (!response.ok) throw new Error(`Employee capacity audit failed with HTTP ${response.status}.`);
const rows = await response.json();
const result = rows?.[0]?.jsonb_build_object;
if (!result) throw new Error("Employee capacity audit returned an unexpected response.");
