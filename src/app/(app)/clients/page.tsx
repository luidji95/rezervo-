...
export default function ClientsPage() {
  ...
  return <div className="clients-page">
    ...
    <div className={`clients-layout ${clients.loading ? "clients-layout--loading" : ""}`}>
      <main className="clients-main"><ClientTable clients={clients.result.items} selectedClient={clients.selectedClient} searchValue={clients.searchValue} status={clients.status} sort={clients.sort} page={clients.result.page} totalPages={clients.result.totalPages} totalCount={clients.result.totalCount} onSearchChange={(value) => { setMobileDetailsOpen(false); clients.setSearchValue(value); }} onStatusChange={(value) => { setMobileDetailsOpen(false); clients.setStatus(value); }} onSortChange={(value) => { setMobileDetailsOpen(false); clients.setSort(value); }} onPageChange={(page) => { setMobileDetailsOpen(false); clients.setPage(page); }} onSelectClient={(client) => { clients.selectClient(client); if (window.matchMedia("(max-width: 767px)").matches) setMobileDetailsOpen(true); }} onDeleteClient={(id) => void clients.handleDeleteClient(id)} onEditClient={(client) => { setEditingClient(editableClient(client)); setIsModalOpen(true); }} /></main>
