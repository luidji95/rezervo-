...
export function getBillingReconciliationConfig(env:Record<string,string|undefined>=process.env){
  ...
  if(env.BILLING_RECONCILIATION_ENABLED!=="true"||env.BILLING_PROVIDER!=="lemonsqueezy"||env.BILLING_ENVIRONMENT!=="test"||!secret||!apiKey||!storeId||!/^\d+$/.test(storeId)) throw new BillingReconciliationConfigError();
