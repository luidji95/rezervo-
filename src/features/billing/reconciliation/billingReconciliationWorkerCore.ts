...
export type BillingReconciliationSummary={claimed:number;inSync:number;remoteNewerEquivalent:number;remoteNewerDrift:number;sameTimestampConflict:number;localNewer:number;localChangedDuringCheck:number;identityConflict:number;mappingConflict:number;planChangeDetected:number;providerNotFound:number;unsupportedRemoteState:number;providerUnavailable:number;configurationError:number;providerResponseInvalid:number;claimLost:number};
...
export async function runBillingReconciliation(input:{runId:string;batchSize:number;storeId:string;repository:BillingReconciliationRepository;provider:BillingReconciliationProvider}){
  const summary=empty();
