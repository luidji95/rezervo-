...
begin
  if exists (
    ...
    where a.outcome = 'recovered_open'
  ) then
    raise exception using
      errcode = '23514',
      message = 'BILLING_CHECKOUT_RECOVERY_LEGACY_RECOVERED_OPEN_FOUND';
  end if;
end;
$$;
...
begin
  ...
  if p_outcome is null or p_outcome not in (
    'already_open', 'already_completed', 'still_pending',
  ...
  if v_attempt.status = 'abandoned' or v_attempt.status <> 'claimed' then
...
begin
  ...
  if p_provider_checkout_id is null or p_provider_checkout_id !~ '^[1-9][0-9]*$' then
  ...
  if v_attempt.provider <> 'lemonsqueezy'
     ...
     or v_checkout.provider_session_id is not null then
    update public.billing_checkout_recovery_attempts a
  ...
  begin
    ...
    where c.id = v_checkout.id
      ...
      and c.provider_session_id is null
...
comment on function public.finalize_billing_checkout_recovery_v1(uuid,uuid,text,text,text,timestamptz) is
  'Token-authoritative atomic creating-to-open checkout recovery finalization; never calls the provider or mutates subscriptions.';
