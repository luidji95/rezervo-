...
-- Lemon Squeezy Checkout resource IDs are canonical lowercase UUID text.
-- Migration 033 used a numeric resource-ID assumption; retain every other
-- token, lease, lock, conflict, audit, and privilege contract unchanged.
...
comment on function public.finalize_billing_checkout_recovery_v1(uuid,uuid,text,text,text,timestamptz) is
  'Token-authoritative atomic creating-to-open checkout recovery finalization using canonical lowercase Lemon Squeezy Checkout UUIDs; never calls the provider or mutates subscriptions.';
