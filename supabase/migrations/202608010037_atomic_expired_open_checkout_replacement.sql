...
comment on function public.acquire_billing_checkout_intent_v1(uuid,uuid,uuid,text,text) is
  'Service-role atomic get-or-create for a server-authorized checkout intent; expired open intents are terminalized under the same advisory lock before acquisition.';
